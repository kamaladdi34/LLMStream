...
function renderMarkdown() {
      ...
      if (!insideCodeBlock) {
        ...
        if (currentBlockType === "tex" || currentBlockType === "latex") {
          ...
          const blockContent = lines
            .slice(i + 1, foundEnd ? endLine : undefined)
            .join("\n");
          const blockId = `latex-block-${i}`;

          const existingBlock = latexBlocks.find(
            (b) => b.content === blockContent,
          );
          if (!existingBlock) {
            latexBlocks.push({
              id: blockId,
  ...
  hasIncompleteMathBlock =
    mathDelimiters.inline.some((d) => d.count % 2 !== 0) ||
  ...
  if (hasIncompleteLatexBlock || hasIncompleteMathBlock) {
    if (latexBlocks.length > 0) {
      ...
      for (const block of latexBlocks) {
        if (!block.isComplete) {
          modifiedLines[block.start] =
            `<div class="latex-generating" id="${block.id}"><span>Generating LaTeX...</span></div>`;
          if (block.start + 1 < modifiedLines.length) {
            modifiedLines.fill(
              "",
              block.start + 1,
              block.end || modifiedLines.length,
            );
          }
        } else {
  ...
  if (
    !hasIncompleteMathBlock &&
    !hasIncompleteLatexBlock &&
    window.lastProcessedContent
  ) {
    let html = md.render(window.markdownContent);
    html = fixMathDelimiters(html);
    html = convertLatexToHTML(html);
    ...
    const lastCommonNewline = findLastCommonNewline(
      window.lastProcessedContent,
      window.markdownContent,
    );

    if (lastCommonNewline !== -1) {
      const commonContent = window.markdownContent.substring(
        0,
        lastCommonNewline,
      );
  ...
  function markdownLatexPlugin(md) {
    ...
    md.renderer.rules.fence = function (tokens, idx, options, env, self) {
      ...
      if (token.info.trim() !== "") {
        ...
        return defaultFence(tokens, idx, options, env, self).replace(
          "<pre>",
          '<pre class="tex2jax_ignore">',
        );
      }
...
function fixMathDelimiters(html) {
  ...
  html = html.replace(
    /<p>\s*\[(.*?\\begin\{cases\}[\s\S]*?\\end\{cases\}.*?)\]\s*<\/p>/gi,
    (match, content) => {
      return `<p>\\[${content}\\]</p>`;
    },
  );
  ...
  html = html.replace(
    /<p>(.*?\\begin\{cases\}[\s\S]*?\\end\{cases\}.*?)<\/p>/gi,
    (match, content) => {
  ...
  html = html.replace(
    /<p>(.*?\\begin\{tcolorbox\}[\s\S]*?\\end\{tcolorbox\}.*?)<\/p>/gi,
    (match, content) => {
      if (content.includes("\\begin{tcolorbox}")) {
        ...
        content = content.replace(
          /(\\begin\{tcolorbox\}(?:\[[\s\S]*?\])?)([\s\S]*?)(\\end\{tcolorbox\})/g,
...
function convertLatexToHTML(text) {
  function processNestedStructures(content) {
    ...
    while (lastContent !== currentContent) {
      ...
      currentContent = currentContent.replace(
        /\\begin\{enumerate\}([\s\S]*?)\\end\{enumerate\}/gs,
        (match, content) => {
      ...
      currentContent = currentContent.replace(
        /\\begin\{itemize\}([\s\S]*?)\\end\{itemize\}/gs,
        (match, content) => {
          let listItems = content.split("\\item").slice(1);
          let htmlList = "<ul>\n";
          listItems.forEach((item) => {
      ...
      currentContent = currentContent.replace(
        /\\begin\{description\}([\s\S]*?)\\end\{description\}/gs,
        (match, content) => {
      ...
      currentContent = currentContent.replace(
        /\\begin\{theorem\}([\s\S]*?)\\end\{theorem\}/gs,
        (match, content) => {
  ...
  text = text.replace(
    /\\begin\{equation\}([\s\S]*?)\\end\{equation\}/gs,
    (match, content) => {
  ...
  text = text.replace(
    /\\begin\{eqnarray\*?\}([\s\S]*?)\\end\{eqnarray\*?\}/gs,
    (match, content, fullMatch) => {
  ...
  text = text.replace(
    /\\begin\{gather\*?\}([\s\S]*?)\\end\{gather\*?\}/gs,
    (match, content, fullMatch) => {
  ...
  text = text.replace(
    /\\begin\{figure\}([\s\S]*?)\\end\{figure\}/gs,
    (match, content) => {
      ...
      const includeGraphicsMatch = content.match(
        /\\includegraphics(?:\[.*?\])?\{(.*?)\}/,
      );
  ...
  text = text.replace(
    /\\begin\{subfigure\}(?:\[.*?\])?\{.*?\}([\s\S]*?)\\end\{subfigure\}/gs,
    (match, content) => {
  ...
  text = text.replace(
    /\\begin\{corollary\}([\s\S]*?)\\end\{corollary\}/gs,
    (match, content) => {
  ...
  text = text.replace(
    /\\begin\{definition\}([\s\S]*?)\\end\{definition\}/gs,
    (match, content) => {
  ...
  text = text.replace(
    /\\begin\{lstlisting\}(\[.*?\])?([\s\S]*?)\\end\{lstlisting\}/gs,
    (match, options, content) => {
      ...
      return `<div class="code-container"><div class="code-title-bar"><span class="language">${language}</span><div class="copy-button" onclick="window.copyCode(this)"></div></div><div class="code-content"><pre><code class="hljs language-${language}">${content.trim()}</code></pre></div></div>`;
    },
  );
  ...
  text = text.replace(
    ...
    (match, placement, content) => {
      ...
      if (tabularMatch) {
        const tabularArgsMatch = tabularMatch[0].match(
        ...
        if (tabularArgsMatch && tabularArgsMatch.length > 2) {
          ...
          const cleanedContent = tabularMatch[0]
  ...
  text = text.replace(
    /\\begin\{longtable\}(\{.*?\})([\s\S]*?)\\end\{longtable\}/gs,
    (match, format, content) => {
      ...
      content = content
        ...
        .replace(/\\hline/g, "");
      let captionMatches = [
        ...content.matchAll(/\\caption(?:\[.*?\])?\{(.*?)\}/g),
      ];
      for (let cm of captionMatches) {
        if (caption === "") {
          caption = cm[1];
        }
        content = content.replace(cm[0], "");
      }
  ...
  function convertTabularContent(content, format) {
    ...
    content = content.replace(/amp;/g, "").replace(/&amp;/g, "&");
    ...
    if (format) {
      ...
      for (let i = 0; i < formatStr.length; i++) {
        switch (formatStr[i]) {
          case "l":
            alignments.push("text-align: left;");
            break;
          case "c":
            alignments.push("text-align: center;");
            break;
          case "r":
            alignments.push("text-align: right;");
            break;
          default:
            alignments.push("");
        }
    ...
    rows.forEach((row, rowIndex) => {
      let rowClass = "";
      if (
        midrulePositions.includes(rowIndex) ||
        midrulePositions.includes(rowIndex - 1)
      ) {
      ...
      cells.forEach((cell, index) => {
        ...
        const style =
          index < alignments.length ? ` style="${alignments[index]}"` : "";
  ...
  text = text.replace(
    /([^\n<>]+)\\hfill([^\n<>]+)/g,
    '<div class="hfill-container"><span>$1</span><span>$2</span></div>',
  );

  text = text.replace(
    /\\begin\{tcolorbox\}(\[[\s\S]*?\])?([\s\S]*?)\\end\{tcolorbox\}/gs,
    (match, options, content) => {
      let title = "";
      let boxColor = "#e6f3ff";
      ...
      let tcolorboxHTML =
        '<div class="tcolorbox" style="background-color: ' +
        boxColor +
        "; border: 1px solid " +
        borderColor +
      ...
      if (title) {
        tcolorboxHTML +=
          '<div class="tcolorbox-title" style="background-color: ' +
          borderColor +
          "; color: " +
          titleColor +
          '; padding: 5px 10px; margin: -15px -15px 10px -15px; border-radius: 5px 5px 0 0; font-weight: bold;">' +
          title +
          "</div>";
      }
      tcolorboxHTML +=
        '<div class="tcolorbox-content">' + content + "</div></div>";
...
function fixOrderedLists(html) {
  const listPattern =
    /<ol>\s*<li>(.*?)<\/li>\s*<\/ol>\s*(<div class="(?:latex-block|code-container)[\s\S]*?<\/div>)\s*<ol start="(\d+)">/g;
  let segments = [];
  let match;
  ...
  if (segments.length > 0) {
    ...
    const completePattern =
      /<ol(?:\s+start="(\d+)")?>(?:\s*<li>(.*?)<\/li>\s*)<\/ol>(?:\s*(<div class="(?:latex-block|code-container)[\s\S]*?<\/div>))?/g;
    ...
    while ((match = completePattern.exec(html)) !== null) {
      ...
      const block = match[3] || "";
      listItems.push({ number: startNum, content: content, block: block });
    }
...
function fixOrderedListsInHTML(html) {
  ...
  while ((match = listWithBlockPattern.exec(html)) !== null) {
    ...
    if (afterHTML.trim()) {
      parts.push({ type: "content", html: afterHTML });
    }
  }
  if (parts.length > 0) {
    let allItems = [];
